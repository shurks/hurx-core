 ...
 * TODO: work this further out, maybe redo it.
 * 
 * @author Stan Hurks
 */
export default class Regex<Aliases extends string = never, OptionalAliases extends string = never, NamedGroups extends string = never> {      
    ...
    public get utils(): {
        helpers: RegexHelpers<Aliases, OptionalAliases, NamedGroups>
        regex: (...args: RegexCallbackFunctionRegexArgParameter<Aliases, OptionalAliases, NamedGroups>[]) => Regex<Aliases, OptionalAliases, NamedGroups>
    ...
    */
     ...
     * @param flags the flags
     */
    public static escape(pattern: string, flags: UniqueCombinations<RegexFlags>): string {
        let temporaryValue = 'x'
        while (pattern.includes(temporaryValue)) {
            temporaryValue += 'x'
        }
        return pattern
            .replace(/\\\\/g, temporaryValue)
 ...
 * Shorthand for regex
 */
export const regex = Regex.alias('any', /./).utils.regex

/**
 * Shorthand for helpers
 */
export const helpers = Regex.alias('any', /./).utils.helpers