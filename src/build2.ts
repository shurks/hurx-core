...
import Env from './library/hurx/env/env'
import { HurxConfig, HurxPaths, HurxPathsPartial } from './library/hurx/hurx-json/hurx-json-file'
import Hurx from './library/hurx/hurx'
import Logger from './library/utils/logger'
import Paths from './library/utils/paths'
import child from 'child_process'
import { env } from './library/tasks/env'
...
const _env = Env.parse(Object.assign({}, JSON.parse(readFileSync(project!).toString('utf8')) as HurxConfig), './')
if (!_env.paths.output) {
    console.error(`No output paths have been found in hurx.json.`)
    process.exit()
}
// Create the dist folder
const distPath = path.join(projectRoot!, _env.paths.output.base)
...
if (!existsSync(path.join(projectRoot!, _env.paths.base, 'tsconfig.json'))) {
    writeFileSync(path.join(projectRoot!, _env.paths.base, 'tsconfig.json'), JSON.stringify({
        "compilerOptions": {
...
const tsconfigJSON = JSON.parse(readFileSync(path.join(projectRoot!, _env.paths.base, 'tsconfig.json')).toString('utf8'))
...
// Package.json
const packageJSONPath = path.join(projectRoot!, _env.paths.base, 'package.json')
const packageJSON = JSON.parse(readFileSync(packageJSONPath).toString('utf8'))
const commands = Hurx.parseCommands()

// Creating the package.json dist
let packageJSONDist = {
    ...packageJSON
}
let packageJSONDistPath = path.join(projectRoot!, _env.paths.output.base, 'package.json')
...
let dist = Paths.relativeWithoutDotSlash(_env.paths.output.base)
...
if (_env.apps.bin) {
    for (const binName of Object.keys(_env.apps.bin)) {
        const appEnv = _env.apps.bin[binName]
        // Install the app
        if (appEnv.npx) {
...
const paths = [[
    ...
    _env.paths.resources, 
    Paths.relativeWithoutDotSlash(_env.paths.output.resources!).replace(dist, '')
], [
    _env.paths.logs, 
    Paths.relativeWithoutDotSlash(_env.paths.output.logs!).replace(dist, '')
]]
for (const _paths of paths) {
    if (_paths[0] && _paths[1]) {
        ...
        if (fs.existsSync(_path)) {
            ...
            if (_paths[0] === _env.paths.sources) {
                logger.info(`No sources folder in project`)
            }
            else if (_paths[0] === _env.paths.resources) {
                logger.info(`No resources folder in project`)
            }
            else if (_paths[0] === _env.paths.logs) {
...
const copyAllFilesExceptTSAndHurx = (_path: string) => {
    const src = path.join(projectRoot!, _env.paths.sources, _path)
...
const copyFilesToActualDist = (_path: string) => {
    ...
    const dist = path.join(projectRoot!, _env.paths.output!.base, _path)
    if (!fs.existsSync(dist)) {
        mkdirSync(dist, {
            recursive: true
        })
