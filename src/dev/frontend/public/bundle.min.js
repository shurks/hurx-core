...
(() => {
  ...
  var _VDOM = class _VDOM {
    ...
    /**
     * Updates the previous node with the current nodes content
     * @param previous the previous node
     * @param current the current node
     * @returns true if everything has changed inside
     */
    updateNode(previous, current, previousVDOM, currentVDOM) {
      var _a, _b;
      const updateEntirely = /* @__PURE__ */ __name((previous2, current2) => {
        if (!previous2.rootElement) {
          throw new Error(`Previous has no root element`);
        }
        console.log({
          previous: previous2
        });
        const indices = _VDOM.getRootElementIndices(previous2, previous2);
        for (let i = indices[indices.length - 1]; i >= indices[0]; i--) {
          previous2.rootElement.removeChild(previous2.rootElement.childNodes[i]);
        }
        if (current2.component) {
          current2.component.vnode = previous2;
          previous2.text = void 0;
          previous2.element = void 0;
          previous2.isFragment = false;
          if (previous2.component) {
            if (previous2.component.constructor.name === current2.component.constructor.name) {
              current2.component.state = _VDOM.deepAssign(current2.component.state, previous2.component.state);
            }
          }
          previous2.component = current2.component;
          current2.renderNode = current2.component.render();
          if (current2.renderNode) {
            _VDOM.createElements(current2.renderNode, null);
            current2.renderNode.renderNodeParent = previous2;
            previous2.renderNode = current2.renderNode;
            _VDOM.mountDOM(previous2.renderNode, previous2);
          }
          previous2.component = current2.component;
          console.log(previous2.component.render());
        } else if (current2.isFragment) {
          previous2.text = void 0;
          previous2.element = void 0;
          previous2.component = void 0;
          previous2.renderNode = void 0;
          previous2.children = current2.children;
          for (let i = 0; i < previous2.children.length; i++) {
            const child = previous2.children[i];
            child.parent = previous2;
            child.rootElement = previous2.rootElement;
          }
        } else if (current2.text) {
          previous2.element = current2.text;
          previous2.component = void 0;
          previous2.renderNode = void 0;
          previous2.children = [];
        } else if (current2.element) {
          previous2.element = current2.element;
          previous2.text = void 0;
          previous2.component = void 0;
          previous2.renderNode = void 0;
          previous2.children = current2.children;
          for (let i = 0; i < previous2.children.length; i++) {
            const child = previous2.children[i];
            child.parent = previous2;
            child.rootElement = previous2.element;
          }
        }
        const elements = _VDOM.getElements(previous2);
        for (let i = 0; i < elements.length; i++) {
          previous2.rootElement.insertBefore(elements[i], previous2.rootElement.childNodes[indices[0] + i]);
        }
      }, "updateEntirely");
      if (current.text) {
      ...
      if (!!current.isFragment !== !!previous.isFragment || !!current.text !== !!previous.text || !!current.element !== !!previous.element || !!current.component !== !!previous.component || !!current.renderNode !== !!previous.renderNode || (current.component && ((_a = current.renderNode) == null ? void 0 : _a.children.length) !== ((_b = previous.renderNode) == null ? void 0 : _b.children.length) || current.children.length !== previous.children.length)) {
        this.logger.verbose("children not same length");
        if (previous === previousVDOM.root) {
          return true;
        }
        updateEntirely(previous, current);
        return true;
      } else {
        if (current.component) {
          ...
          if (current.renderNode && previous.renderNode && current.component.constructor.name === previous.component.constructor.name) {
            if (current.renderNode.children.length !== previous.renderNode.children.length) {
              this.logger.verbose(`Rendernode children are not the same length`);
              if (previous === previousVDOM.root) {
                return true;
              }
              updateEntirely(previous.renderNode, current.renderNode);
              return true;
            } else {
              this.updateNode(previous.renderNode, current.renderNode, previousVDOM, currentVDOM);
            }
          } else if (current.component && previous.component && (current.renderNode === void 0 || previous.renderNode === void 0)) {
            this.logger.verbose({ current, previous });
            throw new Error(`current.renderNode or previous.renderNode is undefined`);
          } else {
            this.logger.verbose("Component is not the same.");
            if (previous === previousVDOM.root) {
              return true;
            }
            updateEntirely(previous, current);
          ...
          for (let i = 0; i < current.children.length; i++) {
            if (!this.updateNode(previous.children[i], current.children[i], previousVDOM, currentVDOM)) {
              allChanged = false;
            }
          }
          return allChanged;
          ...
          if (previous.rootElement) {
            const values = {
              current: current.text instanceof Text ? current.text.textContent : current.text.innerHTML,
              previous: previous.text instanceof Text ? previous.text.textContent : previous.text.innerHTML
            };
            if (values.current === values.previous) {
              return false;
            } else {
              this.logger.verbose("text values changed:", values);
            }
            const indices = _VDOM.getRootElementIndices(previous, previous);
            this.logger.verbose({ textIndices: indices });
            previous.rootElement.removeChild(previous.rootElement.childNodes[indices[0]]);
            if (previous.text instanceof Text) {
              if (current.text instanceof Element) {
                previous.text = _VDOM.document.createElement("span");
                previous.text.innerHTML = current.text.innerHTML;
              } else {
                previous.text.textContent = current.text.textContent;
              }
            } else {
              if (current.text instanceof Text) {
                previous.text = _VDOM.document.createTextNode(current.text.textContent || "");
              } else {
                previous.text.innerHTML = current.text.innerHTML;
              }
            }
            previous.element = previous.text;
            previous.rootElement.insertBefore(previous.element, previous.rootElement.childNodes[indices[0]]);
            return true;
          } else {
    ...
    updateDOM(previous, current) {
      ...
      if (this.updateNode(previous.root, current.root, previous, current)) {
        current.replaceDOM(previous);
      }
    }
    /**
  ...
  var _HomePage = class _HomePage extends Page {
    render() {
      ...
      } }, "homeessssss", /* @__PURE__ */ VDOM.createNode(TestComponent, null));
    }
  };
  ...
  var _TestPage = class _TestPage extends Page {
    render() {
      return /* @__PURE__ */ VDOM.createNode("div", { id: "test", style: "background: yellow; padding: 5px;" }, "Test pagessa", /* @__PURE__ */ VDOM.createNode(TestComponent, null));
    }
  };
  ...
  var _A = class _A extends Component {
    ...
    render() {
      return /* @__PURE__ */ VDOM.createNode(VDOM.createNode, null, /* @__PURE__ */ VDOM.createNode("div", { id: this.state.test, onClick: () => {
        this.state.test++;
      } }, this.state.test, /* @__PURE__ */ VDOM.createNode("div", null, "asasssaass"), /* @__PURE__ */ VDOM.createNode("div", null, "fessf"), /* @__PURE__ */ VDOM.createNode("div", null, "1ss"), /* @__PURE__ */ VDOM.createNode("div", null, "3"), /* @__PURE__ */ VDOM.createNode("div", null, "4"), /* @__PURE__ */ VDOM.createNode("div", null, "gga2b")), /* @__PURE__ */ VDOM.createNode("div", null, "Yoloa"), "Test");
    }
  };
  __name(_A, "A");
  var A = _A;
  new VDOM(
    "main",
    /* @__PURE__ */ VDOM.createNode("div", null, /* @__PURE__ */ VDOM.createNode("div", { style: "background: red; color: white; padding: 5px;" }, /* @__PURE__ */ VDOM.createNode("div", null, "Start of VDOM")), /* @__PURE__ */ VDOM.createNode("div", null, "Tessts"), /* @__PURE__ */ VDOM.createNode(VDOM.createNode, null, /* @__PURE__ */ VDOM.createNode(Router, null, /* @__PURE__ */ VDOM.createNode("div", null, "Route tes12t"), /* @__PURE__ */ VDOM.createNode("div", null, "Route tes12t"), /* @__PURE__ */ VDOM.createNode("div", null, "Route tes12t"), /* @__PURE__ */ VDOM.createNode("div", null, "Route tes12ts"), /* @__PURE__ */ VDOM.createNode(A, null), "test2", /* @__PURE__ */ VDOM.createNode(Route, { path: "/test" }, /* @__PURE__ */ VDOM.createNode(TestPage, null), /* @__PURE__ */ VDOM.createNode(TestPage, null), /* @__PURE__ */ VDOM.createNode(TestPage, null)), "test12345")), /* @__PURE__ */ VDOM.createNode("div", { style: "background: maroon; color: white; padding: 5px;" }, /* @__PURE__ */ VDOM.createNode("div", null, "End of VDOM")))
  );
})();
